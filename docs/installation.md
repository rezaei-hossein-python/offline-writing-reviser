...
6. Setup reuses `qwen3:1.7b` or downloads only that model through Ollama, verifies the model list, runs a minimal inference, and completes a LanguageTool + Qwen semantic revision test.
...
The model is not bundled in the installer. Allow approximately 3 GB of free space for Ollama, the model, and headroom. Setup time depends on internet speed. First inference may be slower while the model loads. CPU-only machines work but are slower; Ollama may use a supported GPU automatically.

## Upgrade, reinstall, and uninstall

Run the newer installer over the existing per-user installation. Setup stops the old background process, replaces application files, preserves settings and logs, restores the HKCU startup entry, and reuses Ollama. Model Setup verifies or downloads `qwen3:1.7b`, performs minimal inference and an end-to-end semantic test, switches the saved default, and only then removes the exact former `gemma3:4b` model. A failure keeps Gemma and the prior working configuration and shows **Retry**. Unrelated models are never removed. No manual Ollama command is required.
...
| Symptom | Resolution |
|---|---|
| Ollama is installed but the model is missing | Open **Set up intelligent revision**. Setup skips Ollama installation and pulls only `qwen3:1.7b`. |
...
| AI model not ready | Finish Model Setup. Ready appears only after Ollama, model-list verification, minimal inference, and the end-to-end semantic test all pass. |
